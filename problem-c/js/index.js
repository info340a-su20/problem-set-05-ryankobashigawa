...
function createTaskItemElement(task) {
  ...
  list.addEventListener('click', function() {
    ...
    return renderTaskList(state.taskList);
  })
  return list;
}
//let testing = createTaskItemElement(state.taskList[1]);
//console.log(testing);
...
function renderTaskList(ol) {
  let fillList = document.querySelector('ol'); // get <ol>
  fillList.innerHTML = "";                     //  remove previous list content
  for(let i = 0; i < ol.length; i++) {         // go through list
    fillList.appendChild(createTaskItemElement(ol[i])); // add to list
  }
  renderInput(); // add renderInput
}
...
input.addEventListener('input', function() {
  let user = document.querySelector('input').value; // get user input
  state.inputtedText = user; // change state's input to user input
  renderInput(); // add renderInput
})
