...
function getElementWidth(dom) {
  let rect = dom.getBoundingClientRect().width;
  return rect;
}
// test getElementWidth

// let test = document.querySelector('img');
// let testo = test.innerHTML;
// console.log(testo);
// let w = getElementWidth(test);
// console.log(w);
...
function renderPaletteRow(array, parent) {
  let newDivElem = document.createElement('div'); //create div: row of colored boxes
  let width = getElementWidth(parent) / array.length; // get equal sizes
  for(let i = 0; i < array.length; i++) { //traverse thru array
    let colorDiv = createColorBox(array[i], width); //create div for each element in array, equal size 
    newDivElem.appendChild(colorDiv); //append color box to container div
  }
  parent.appendChild(newDivElem); //append container div to parent
}
// test renderpalleterow
// let test = document.querySelector('main'); 
// let testelem = test.innerHTML; // get main element itself
// let test1 = renderPaletteRow(COLORS_9.Reds, test);
// console.log(test1);
...
// function renderPalleteTable() {
//console.log(COLORS_9.Reds);
